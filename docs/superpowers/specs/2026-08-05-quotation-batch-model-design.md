...
| Screenshots of `accounting-system-351b1.web.app` | Authoritative structure reference covering batch/item/specification nesting, modal inventory, column layout. |
...
`QuotationLineItem` gains optional `batchId` and `itemId`. Every existing consumer keeps reading `items` and needs no change: `QuotationDetail`, `InvoiceDetail`, `convertToSalesOrder`, Reports and the commercial invoice. Seeded quotations that have no `batches` render exactly as they do today.
...
- §5.1 row 1 base case: Given 5.0000, no rules, no extras → `U/P 2,475.00`, `AMOUNT 24,750.00` at qty 10, `WEIGHT 4,950.00` (guards §6.2.1, since U/P must not be gated on opening the pricing modal)
