...
This is a standard Vite + React static site — Vercel auto-detects it.
...
- Palette, typography, and the net-mesh watermark motif are pulled directly from the Fortune
  ...
  colors — navy doubles as the "approved/complete" status tone, same convention as before;
...
- The "Mark Step Complete" action on Sales Orders is a simplified stand-in for what will be
  ...
  the full spec) — sufficient for demonstrating the lifecycle and payment-gating logic, not a
...
3. **Sales Orders → SO-1044** — the central workspace. Point out the lifecycle stepper, the
   ...
   verify the balance payment on the Payments tab — watch the Container Loading control flip
