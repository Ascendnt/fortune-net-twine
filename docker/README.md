...
It's here so that when you're ready to start the "Next" horizon, meaning a real backend, a
database, and the Inquiry→PI→PO→Sales Order flow with enforced business rules, you have a running
...
this, so add one), and don't expose port 5432 publicly.

## Wiring up a real backend (suggested shape)

Per the roadmap's Phase 1 stack of a Node.js backend, PostgreSQL and shared TypeScript types:
...
container directly, because Vercel doesn't host stateful containers. For a hosted Phase 1 demo
...
- [Neon](https://neon.tech) or [Supabase](https://supabase.com) both offer a free Postgres
  tier with a connection string you can paste into Vercel's environment variables, with no
