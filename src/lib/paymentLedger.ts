 ...
 * be rejected — customers do round up, or pay two invoices in one transfer — it is an overpayment,
 * and it has to be visible rather than silently absorbed.
 */
...
export interface OrderLedger {
  ...
  expected: number;
  received: number;
  /** Total still owed across the order. Never negative — an overpayment is reported separately. */
  outstanding: number;
  overpaid: number;
 ...
 *
 * Deposit first, then balance, then anything else — the order money is normally asked for in. This
 * is what lets the dialog open with the right line and the right amount already filled in.
 *
 * Skips a line still pending approval. `canRecordAgainst` blocks it at the row level, and this is
 * the same shortcut a "Record payment" button reaches for — it must not hand back a line the dialog
 * it opens is not allowed to touch.
 */
 ...
 * blocked as before. The balance is the opposite — customers pay it down as funds allow, and
 ...
 *
 * Returns undefined when the payment settles nothing the order is currently waiting on — a
